...
# 批次查詢多個群組的專案列表 🆕
uv run python gl-cli.py user-projects --group-name "group1" "group2"
```

**主要功能：**
- ✅ `user-details` 支援多筆 `--username` 和 `--project-name`
- ✅ `user-projects` 支援多筆 `--username` 和 `--group-name`
- ✅ 笛卡爾積邏輯（多使用者 × 多專案/群組自動組合）
- ✅ 清晰的進度顯示（查詢 X/Y）
- ✅ 向下相容（單一值仍正常運作）
...
# 查詢特定群組的使用者專案列表
uv run python gl-cli.py user-projects --group-name "my-group"
...
# 批次查詢多個群組的專案列表 🆕
uv run python gl-cli.py user-projects --group-name "group1" "group2"
# 會執行 2 次查詢，產生 4 個檔案（2個群組 × 2種類型）

# 組合查詢：多位使用者在多個群組的專案 🆕
uv run python gl-cli.py user-projects --username alice bob --group-name "group1" "group2"
# 會執行 4 次查詢（2位使用者 × 2個群組），產生 8 個檔案
```
...
5. **多筆處理** - 支援多使用者和多群組的批次查詢，顯示進度（查詢 X/Y）🆕
6. **笛卡爾積邏輯** - 多使用者 × 多群組自動組合查詢 🆕
7. **錯誤提示** - 若無資料則提供可能原因和建議

**查詢參數：**
- `--username` - 使用者名稱（可選，不填則查詢所有使用者；**支援多筆**，例如：`alice bob charlie`）🆕
- `--group-name` - 群組名稱（可選，用於限制在特定群組的專案；**支援多筆**，例如：`group1 group2`）🆕
...
# 批次查詢多個群組的專案權限
uv run python gl-cli.py user-projects --group-name "engineering" "operations"
# 會執行 2 次查詢，產生 4 個檔案

# 組合查詢：特定使用者在多個群組的專案
uv run python gl-cli.py user-projects --username alice --group-name "group1" "group2"
# 會執行 2 次查詢（1位使用者 × 2個群組），產生 4 個檔案
```
...
- ✨ 笛卡爾積邏輯 - 多使用者 × 多專案/群組自動組合查詢
- ✨ 進度顯示 - 清晰顯示當前查詢進度（查詢 X/Y）
- 🎯 實際應用：團隊績效評估、專案健康檢查、跨專案貢獻分析、批次權限盤點、跨群組權限審計

**user-projects 參數調整：**
- ✅ `--username` 支援多筆參數（可指定多個使用者名稱）
- ✅ `--group-id` 改為 `--group-name`，使用群組名稱而非 ID，並支援多筆參數
