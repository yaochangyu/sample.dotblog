...
在 RTX 4060 Laptop（8 GB VRAM）上，透過 **Ollama + LiteLLM** 讓 Claude Code CLI 改用地端 `gemma4:e4b` 模型推論的完整實驗紀錄。
...
```bash
cd /mnt/d/lab/sample.dotblog/Local-LLM/claude-gemma4
bash script/run-claude.sh
```
...
| 檔案 | 用途 |
|------|------|
| `script/run-claude.sh` | 一鍵啟動腳本（主要入口） |
| `script/start-optimized-env.sh` | 環境啟動層（Ollama + LiteLLM） |
| `script/Modelfile` | 定義 gemma4-opt 模型參數 |
| `script/litellm_config.yaml` | LiteLLM proxy 設定 |
| `script/empty-mcp.json` | 停用 MCP servers 用的空設定檔 |
| `script/capture_proxy.py` | HTTP logging proxy，診斷 token 用量 |
| `blog.md` | 本次實驗的部落格踩坑紀錄（已完成最佳化與用語在地化） |
...
| `.archive/optimization-report.md` | 完整最佳化過程與資料（已封存） |
...
預設啟用 MCP 時（GitHub、Gmail、Context7 等），tool 數從 28 增加至 59，輸入 tokens 從 ~29K 飆升至 ~54K，完全超出任何能在 8 GB VRAM 執行的地端模型上限。
