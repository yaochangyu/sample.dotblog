...
> ⚠️ **此做法的安全風險與局限性**
> 
> * **非物理性強制力（軟性約束）**：不論是 System Prompt 行為約束，還是寫在規則檔中的開發安全守則，都屬於**軟性約束**。AI Agent 在遭受精心設計的 Prompt Injection（如角色扮演、越獄攻擊）時，這些規定很有可能會被 AI 拋在腦後，在執行時繞過限制，因此絕不能單靠這招當作唯一防線。
