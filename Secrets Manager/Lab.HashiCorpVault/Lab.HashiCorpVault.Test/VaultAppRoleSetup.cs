...
public class VaultAppRoleSetup
{
    ...
    private readonly Dictionary<string, string> _appRolePolicies = new()
    {
        ["app-dev"] = """
    ...
    public async Task SetupVaultAsync()
    {
        ...
        try
        {
            ...
            foreach (var (policyName, policyContent) in _appRolePolicies)
            {
            ...
            foreach (var (roleName, _) in _appRolePolicies)
            {
                // 建立 AppRole
                await ExecuteVaultCommandAsync($"""
                    write auth/approle/role/{roleName} \
                ...
                // 產生 Secret ID
                var secretIdResult = await ExecuteVaultCommandAsync($"write -format=json auth/approle/role/{roleName}/secret-id");
                ...
                // 測試使用 AppRole 登入
                await TestAppRoleLogin(roleName);
            }
        }
        catch (Exception e)
        {
            Console.WriteLine($"Error during AppRole setup: {e.Message}");
        }
    }

    private async Task TestAppRoleLogin(string roleName)
    {
        try
        {
            ...
            // 使用 AppRole 登入
            var loginResult = await ExecuteVaultCommandAsync($"""
