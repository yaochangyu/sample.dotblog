...
- [x] 階段 5 完成 (2025-10-06)
  - 程式碼行數：從 1977 行減少到 1913 行（減少 64 行，約 3.2%）
  - 編譯結果：成功，0 警告 0 錯誤
  - 主要改善：
    - 完成 10 個資料庫操作方法改用 DatabaseHelper
    - 查詢類方法 5 個：LoadMarkedFiles (25→16行)、LoadMarkedFilesByHash (32→23行)、LoadSkippedHashes (28→16行)、LoadMarkedForDeletionFilesWithDetails (24→16行)、LoadExistingHashes (38→31行)
    - 單筆操作方法 1 個：MarkFileForDeletion (27→10行)
    - 批次操作方法 4 個：MarkHashAsSkipped (34→24行)、UnskipHashes (20→17行)、UnmarkFiles (20→17行)、WriteToDatabase (71→54行)
    - 特殊方法評估：ExecuteMarkedDeletions 和 InitializeDatabase 維持原樣（複雜邏輯與 DDL 操作特性）
    - DatabaseHelper 使用率：從 3 處增加到 13 處
    - 消除了所有資料庫連線建立、參數綁定的重複程式碼
  - 實際效果：
    - 統一資料庫存取層：所有 CRUD 操作都使用 DatabaseHelper
    - 提升可維護性：修改資料庫連線邏輯只需改 DatabaseHelper
    - 提升一致性：統一的錯誤處理、連線管理、交易處理
    - 提升可讀性：業務邏輯與資料庫操作分離更清晰
...
- [x] `LoadMarkedFiles()` (1144-1168行)
  - [x] 改用 `DatabaseHelper.ExecuteQuery()`
  - [x] 移除手動連線建立
  - [x] 測試查詢結果正確性
  - [x] 實際: 從 25 行 → 16 行

- [x] `LoadMarkedFilesByHash()` (1170-1202行)
  - [x] 改用 `DatabaseHelper.ExecuteQuery()`
  - [x] 簡化字典建立邏輯
  - [x] 測試分組結果正確性
  - [x] 實際: 從 32 行 → 23 行

- [x] `LoadSkippedHashes()` (1204-1228行)
  - [x] 改用 `DatabaseHelper.ExecuteQuery()`
  - [x] 移除 try-catch 重複邏輯
  - [x] 測試 HashSet 正確性
  - [x] 實際: 從 28 行 → 16 行

- [x] `LoadMarkedForDeletionFilesWithDetails()` (1086-1110行)
  - [x] 改用 `DatabaseHelper.ExecuteQuery()`
  - [x] 簡化 tuple 清單建立
  - [x] 測試詳細資訊正確性
  - [x] 實際: 從 24 行 → 16 行

- [x] `LoadExistingHashes()` (1786-1824行)
  - [x] 改用 `DatabaseHelper.ExecuteQuery()`
  - [x] 保持雙字典回傳邏輯
  - [x] 測試檔案過濾邏輯
  - [x] 實際: 從 38 行 → 31 行
...
- [x] `MarkFileForDeletion()` (649-676行)
  - [x] 改用 `DatabaseHelper.ExecuteNonQuery()`
  - [x] 使用 `SqliteParameter` 簡化參數綁定
  - [x] 測試插入操作正確性
  - [x] 實際: 從 27 行 → 10 行

### 5.3 批次操作類方法改寫 (4 個方法)

- [x] `MarkHashAsSkipped()` (1230-1264行)
  - [x] 改用 `DatabaseHelper.ExecuteTransaction()`
  - [x] 簡化批次插入邏輯
  - [x] 測試交易正確性
  - [x] 實際: 從 34 行 → 24 行

- [x] `UnskipHashes()` (1271-1291行)
  - [x] 改用 `DatabaseHelper.ExecuteTransaction()`
  - [x] 簡化批次刪除邏輯
  - [x] 測試交易回滾
  - [x] 實際: 從 20 行 → 17 行

- [x] `UnmarkFiles()` (1117-1137行)
  - [x] 改用 `DatabaseHelper.ExecuteTransaction()`
  - [x] 移除手動交易管理
  - [x] 測試批次取消標記
  - [x] 實際: 從 20 行 → 17 行

- [x] `WriteToDatabase()` (1906-1977行)
  - [x] 改用 `DatabaseHelper.ExecuteTransaction()`
  - [x] 簡化大量插入邏輯
  - [x] 測試批次寫入效能
  - [x] 實際: 從 71 行 → 54 行
...
- [x] `ExecuteMarkedDeletions()` (682-802行)
  - [x] 評估不需要改寫（已使用手動連線管理處理複雜邏輯）
  - [x] 保留複雜的刪除邏輯
  - [x] 測試完整刪除流程
  - [x] 實際: 未改寫（維持原樣）

- [x] `InitializeDatabase()` (1864-1901行)
  - [x] 評估不改用 `DatabaseHelper`
  - [x] DDL 操作保持原樣
  - [x] 測試資料庫初始化
  - [x] 實際: 未改寫（DDL 操作特性）
...
**階段 5 實際總效果**: 減少 **64 行** (從 1977 行 → 1913 行，約 3.2%)
