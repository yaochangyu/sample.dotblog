...
| 推薦等級 | 資料型別分類 | 實作架構 | API 端點 | 總耗時<br>(ms) | GC 總停頓時間<br>(Pause Time / 佔比) | Gen0 GC<br>次數 | Gen1 GC<br>次數 | Gen2 GC<br>次數 | LOH 峰值<br>(MB) | Working Set<br>實體記憶體 | 核心評語與行為特徵 |
|:---:|:---|:---|:---|:---:|:---:|:---:|:---:|:---:|:---:|:---:|:---|
| 🏆 **S 級** | **1. 原生數值**<br>*(double 4MB)* | **Streaming (串流回傳)** | `/api/export-readings-stream` | **787** | **0.0 ms (0.99%)** | **0 次** | **0 次** | ✅ **0 次 (零 GC)** | **2 MB** | **223 MB** | 🏆 **0 LOH、零 GC、邊產邊傳記憶體平穩** |
| ⚡ **A 級** | **1. 原生數值**<br>*(double 4MB)* | **ArrayPool (池化回傳)** | `/api/export-readings` | 492 | 14.6 ms (0.91%) | 2 次 | 2 次 | ⚠️ 2 次 (常規) | 2 MB | 223 MB | ⚡ 租用 4MB Buffer 序列化後歸還 |
| ❌ **D 級** | **1. 原生數值**<br>*(double 4MB)* | **List (未池化回傳)** | `/api/export-readings-list` | 707 | 29.8 ms (0.87%) | 2 次 | 1 次 | ⚠️ 1 次 (常規) | 2 MB | 218 MB | ❌ 每次請求建立大 List 佔據 LOH |
| 🏆 **S 級** | **2. 原生字串**<br>*(string 50k 筆)* | **Streaming (串流回傳)** | `/api/export-strings-stream` | **304** | **67.5 ms (0.87%)** | 10 次 | **2 次** | ✅ **1 次 (常規)** | **2 MB** | **222 MB** | 🏆 **最快、0 LOH、停頓極短** |
| ⚠️ **C 級** | **2. 原生字串**<br>*(string 50k 筆)* | **ArrayPool (池化回傳)** | `/api/export-strings` | 275 | 127.0 ms (0.90%) | 9 次 | 1 次 | ❌ 0 次 (劇烈) | 2 MB | 222 MB | ⚠️ 僅池化指標陣列，5 萬字串仍在 Gen0 產垃圾 |
| ❌ **D 級** | **2. 原生字串**<br>*(string 50k 筆)* | **List (未池化回傳)** | `/api/export-strings-list` | 318 | 170.5 ms (0.95%) | 10 次 | 6 次 | ❌ 2 次 (劇烈) | 2 MB | 221 MB | ❌ 5 萬字串大 List 衝入 LOH |
| 🏆 **S 級** | **3. 巢狀結構**<br>*(Struct 20k 筆)* | **Streaming (串流回傳)** | `/api/export-members-stream` | **538** | **119.7 ms (0.98%)** | 7 次 | **2 次** | ✅ **1 次 (常規)** | **2 MB** | **225 MB** | 🏆 **最快、0 LOH、停頓短、記憶體減半** |
| ⚡ **A 級** | **3. 巢狀結構**<br>*(Struct 20k 筆)* | **ArrayPool (池化回傳)** | `/api/export-members` | 533 | 193.1 ms (1.03%) | 16 次 | 7 次 | ⚠️ 3 次 (常規) | 2 MB | 226 MB | ⚡ 租用 Buffer 序列化後歸還 |
| ❌ **D 級** | **3. 巢狀結構**<br>*(Struct 20k 筆)* | **List (未池化回傳)** | `/api/export-members-list` | 552 | 196.8 ms (1.08%) | 7 次 | 3 次 | ❌ 1 次 (劇烈) | 2 MB | 225 MB | ❌ 20k Struct List 進入 LOH |
| 🛡️ **B 級** | **4. 參考型別**<br>*(Class 20k 筆)* | **Streaming (串流回傳)** | `/api/export-members-class-stream` | **326** | **56.3 ms (1.07%)** | 6 次 | **1 次** | ✅ **0 次 (極低)** | **2 MB** | **226 MB** | 🏆 **Class 最佳解，0 LOH** |
| ⚠️ **C 級** | **4. 參考型別**<br>*(Class 20k 筆)* | **ArrayPool (池化回傳)** | `/api/export-members-class-pooled` | 446 | 178.3 ms (1.11%) | 17 次 | 6 次 | ❌ 4 次 (劇烈) | 2 MB | 223 MB | ⚠️ 池化效益低，Class 物件觸發 GC |
| ❌ **D 級** | **4. 參考型別**<br>*(Class 20k 筆)* | **List (未池化回傳)** | `/api/export-members-class-list` | 510 | 204.6 ms (1.16%) | 5 次 | 2 次 | ❌ 1 次 (劇烈) | 2 MB | 221 MB | ❌ 20k Class List 佔據 LOH |

### 核心發現：
1. **`IAsyncEnumerable<T>` 串流輸出在 Response 端全面制霸（🏆 S 級）**：
   - 不論是數值、字串、Struct 或 Class，串流回傳均達成 **全程 0 LOH 配置**。
   - 實體記憶體 Working Set 穩定壓在 **92~223 MB**，且處理速度最快。
2. **Response 端的 ArrayPool / List 易在 Buffer 序列化時產生瞬時 LOH 尖峰**：
   - 當端點持有數十萬筆集合傳給 `JsonSerializer.SerializeAsync` 時，若序列化器在輸出管線中持有大塊 Buffer，會導致 LOH 與 Working Set 上升。因此回傳大型集合時，**首選始終是 `IAsyncEnumerable<T>` 串流輸出**。
...
|:---:|:---|:---|:---:|:---:|:---:|:---:|:---:|:---:|:---:|:---:|:---|
| 🏆 **S 級** | **1. 原生數值**<br>*(double 4MB)* | **Streaming (串流接收)** | **3,524** | **104.31 ms (2.94%)** | 67 次 | 1 次 | ✅ **0 次 (零 Gen2)** | **0 MB** | **2 MB** | **222 MB** | 🏆 **Client 0 LOH、無大陣列擴容** |
| ❌ **D 級** | **1. 原生數值**<br>*(double 4MB)* | **List (未池化接收)** | 1,827 | 24.69 ms (1.71%) | 8 次 | 8 次 | ❌ 8 次 (劇烈) | **42.25 MB** | 2 MB | 215 MB | ❌ **Client 每次 new 4MB double[] 砸進 LOH** |
| 🏆 **S 級** | **2. 原生字串**<br>*(string 50k)* | **Streaming (串流接收)** | **577** | **59.39 ms (9.87%)** | 25 次 | 3 次 | ✅ **1 次 (極低)** | **0 MB** | **2 MB** | **212 MB** | 🏆 **Client 逐筆消費 0 LOH** |
| ❌ **D 級** | **2. 原生字串**<br>*(string 50k)* | **List (未池化接收)** | 1,005 | **287.60 ms (27.12%)** | 32 次 | 31 次 | ❌ 10 次 (劇烈) | **9.88 MB** | 2 MB | 214 MB | ❌ **GC 停頓佔 27.1%，頻繁 Gen2 GC** |
| 🏆 **S 級** | **3. 巢狀結構**<br>*(Struct 20k)* | **Streaming (串流接收)** | **1,559** | **62.17 ms (3.86%)** | 28 次 | 1 次 | ✅ **0 次 (零 Gen2)** | **0 MB** | **2 MB** | **214 MB** | 🏆 **Client 0 LOH、記憶體佔用極小** |
| ❌ **D 級** | **3. 巢狀結構**<br>*(Struct 20k)* | **List (未池化接收)** | 2,256 | **909.16 ms (39.46%)** | 36 次 | 34 次 | ❌ 15 次 (劇烈) | **28.75 MB** | 2 MB | 216 MB | ❌ **GC 停頓高達 39.46% (909ms)** |
| 🛡️ **B 級** | **4. 參考型別**<br>*(Class 20k)* | **Streaming (串流接收)** | **1,263** | **42.30 ms (3.35%)** | 23 次 | 1 次 | ✅ **0 次 (零 Gen2)** | **0 MB** | **2 MB** | **215 MB** | 🏆 **Client 0 LOH、停頓極短** |
| ❌ **D 級** | **4. 參考型別**<br>*(Class 20k)* | **List (未池化接收)** | 2,144 | **770.70 ms (35.11%)** | 31 次 | 30 次 | ❌ 9 次 (劇烈) | **4.38 MB** | 2 MB | 217 MB | ❌ **GC 停頓高達 35.11% (770ms)** |
