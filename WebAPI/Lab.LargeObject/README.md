...
| 排名梯隊 | 架構寫法組合 | LOH 配置 | Gen2 GC 次數 | GC 總停頓時間 | Working Set 記憶體 | 處理耗時 | 綜合評語 |
|:---|:---|:---:|:---:|:---:|:---:|:---:|:---|
| 🥇 **S 級 (最優解)** | **`Struct` + `IAsyncEnumerable<T>` 串流解析** | **0 MB** | **16~24 次 (常規)** | **10.6~58.8 ms (極短)** | **114~132 MB (減半)** | **3.3~3.6s (最快)** | 🏆 **效能與記憶體雙冠王**，全程 0 大物件、零暖機成本 |
| 🥈 **A 級 (特定首選)** | **`Struct` + `ArrayPool<T>` 自訂池化** | 62~118 MB | **10~19 次 (暖機後 0)** | 61.2~107.4 ms | 276~325 MB (常駐) | 3.2~4.4s | ⚡ **需隨機存取陣列時的首選**，Gen0 暴降 99%，零後續 GC 停頓 |
| 🥉 **B 級 (折衷方案)** | **`Class` + `IAsyncEnumerable<T>` 串流解析** | **0 MB** | **17~27 次 (常規)** | **55.4 ms (極短)** | **140 MB (減半)** | 4.4s (良好) | 🛡️ **既有 Class 模型無法改為 struct 時的最佳解** |
| ⚠️ **C 級 (效益極低)** | **`Class` + `ArrayPool<T>` 池化** | 6 MB | 67~164 次 (劇烈) | 137.1 ms (偏長) | 258~351 MB (偏高) | 3.1~3.3s | ❌ **白做工**，只池化到指標，物件實體依舊在 Gen0 瘋狂產出垃圾 |
| 🚫 **D 級 (強烈禁止)** | **`Struct` / `Class` + 直接用 `List<T>` 接收** | 31~62 MB | **60~202 次 (極高)** | **182.7~246.3 ms (極長)** | 196~252 MB | 3.1~4.8s (抖動) | 💥 **效能毒藥**，擴容放大效應引發頻繁 Gen2 GC 與 OOM 風險 |
