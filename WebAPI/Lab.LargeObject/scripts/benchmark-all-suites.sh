...
# 參數支援：
#   ./benchmark-all-suites.sh           # 一鍵完整重跑全部 3 大壓測套件並持久化
#   ./benchmark-all-suites.sh --report  # 一鍵輸出全部 3 大套件的 Markdown 彙總大表（0.1 秒秒級重用）
...
if [[ "${1:-}" == "--report" || "${1:-}" == "-r" ]]; then
    ...
    echo "================================================================================"
    echo ""
    "$SCRIPT_DIR/benchmark-all-12.sh" --report
    echo ""
    echo "--------------------------------------------------------------------------------"
    echo ""
...
"$SCRIPT_DIR/benchmark-all-12.sh"
echo "✅ Request 12 組完成！"
echo ""
...
echo "👉 後續可隨時執行 ./scripts/benchmark-all-suites.sh --report 查看完整大表"
echo "================================================================================"
