...
# 12 種全組合一鍵自動化大橫評腳本（4 種資料型別 × 3 種架構，支援結果持久化與秒級重用）：
...
render_markdown_table() {
    ...
    echo "## 實測數據：12 種全組合完整對照大一統總表（4 種資料型別 × 3 種架構）"
    echo ""
...
if [[ ! -f "$PAYLOAD_STRINGS" ]]; then
    awk 'BEGIN{
        printf "[";
        for (i=0;i<50000;i++){
            if(i>0)printf ",";
            printf "\"sample-log-record-uuid-0000-0000-000000000000-%08d-dotblog-benchmark\"", i;
        }
        printf "]";
    }' > "$PAYLOAD_STRINGS"
fi
...
run_test() {
    ...
    local verdict="$6"
    local csv_out="$SCRIPT_DIR/bench-12-${type_category}-${pattern_name}-$(date +%s).csv"
...
echo "=== 開始 12 種全組合橫評測試 (50 請求，10 並行) ==="
...
run_test "🏆 S 級" "2. 原生字串 (string 50k)" "Streaming (串流)" "/api/strings-stream" "$PAYLOAD_STRINGS" "🏆 String 最佳解，0 LOH、停頓極短、記憶體極低"
run_test "⚠️ C 級" "2. 原生字串 (string 50k)" "ArrayPool (池化)" "/api/strings" "$PAYLOAD_STRINGS" "⚠️ 池化效益低，僅池化指標陣列，字串實體仍散落 Gen0"
run_test "❌ D 級" "2. 原生字串 (string 50k)" "List (未池化)" "/api/strings-list" "$PAYLOAD_STRINGS" "❌ 擴容指標陣列衝破 85KB LOH，且引發高頻 GC"

# 3. 結構值型別 (Struct 20k 筆)
run_test "🏆 S 級" "3. 巢狀結構 (Struct 20k)" "Streaming (串流)" "/api/members-stream" "$PAYLOAD_MEMBERS" "🏆 Struct 最佳解，停頓降 70%、0 LOH、記憶體減半"
run_test "⚡ A 級" "3. 巢狀結構 (Struct 20k)" "ArrayPool (池化)" "/api/members" "$PAYLOAD_MEMBERS" "⚡ 需隨機存取首選，資料內嵌於 Buffer 完整池化"
run_test "❌ D 級" "3. 巢狀結構 (Struct 20k)" "List (未池化)" "/api/members-list" "$PAYLOAD_MEMBERS" "❌ GC 停頓極長 (218ms)，短命陣列引發頻繁 Full GC"

# 4. 參考型別 (Class 20k 筆)
run_test "🛡️ B 級" "4. 參考型別 (Class 20k)" "Streaming (串流)" "/api/members-class-stream" "$PAYLOAD_MEMBERS" "🏆 Class 最佳解，GC 停頓降 66%，記憶體維持極低"
run_test "⚠️ C 級" "4. 參考型別 (Class 20k)" "ArrayPool (池化)" "/api/members-class-pooled" "$PAYLOAD_MEMBERS" "⚠️ 池化效益低，僅省下指標，物件依舊觸發長時間 GC"
run_test "❌ D 級" "4. 參考型別 (Class 20k)" "List (未池化)" "/api/members-class-list" "$PAYLOAD_MEMBERS" "⚠️ 4 萬個 Class 實體散落 Gen0，GC 停頓高達 173ms"
