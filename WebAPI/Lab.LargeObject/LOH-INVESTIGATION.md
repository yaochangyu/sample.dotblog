...
| 推薦等級 | 資料型別分類 | 實作架構 | API 端點 | 總耗時<br>(ms) | GC 總停頓時間<br>(Pause Time / 佔比) | Gen0 GC<br>次數 | Gen2 GC<br>次數 | LOH 峰值<br>(MB) | Working Set<br>實體記憶體 | 核心評語與行為特徵 |
|:---:|:---|:---|:---|:---:|:---:|:---:|:---:|:---:|:---:|:---|
| 🏆 **S 級** | **1. 原生數值**<br>*(double 4MB)* | **Streaming (串流)** | `/api/readings-stream` | **3,665** | **10.6 ms (0.33%)** | **1.3M** | ✅ **19 次 (常規)** | **0 MB** | **114 MB** | 🏆 **最快、停頓最短 (10ms)、記憶體最低** |
| ⚡ **A 級** | **1. 原生數值**<br>*(double 4MB)* | **ArrayPool (池化)** | `/api/readings` | 4,483 | 61.2 ms (1.14%) | 75.5M | ⚠️ 19 次 (常規) | 118 MB | 325 MB | ⚡ 陣列完整池化，暖機後重複複用 4MB Buffer |
| ❌ **D 級** | **1. 原生數值**<br>*(double 4MB)* | **List (未池化)** | `/api/readings-list` | 4,831 | 59.9 ms (0.87%) | 22.9M | ⚠️ 17 次 (常規) | 62 MB | 196 MB | ❌ 擴容連續拋棄暫存陣列，製造 LOH 垃圾 |
| 🏆 **S 級** | **2. 巢狀結構**<br>*(Struct 20k 筆)* | **Streaming (串流)** | `/api/members-stream` | **3,329** | **58.8 ms (1.03%)** | **6.0M** | ✅ **16 次 (常規)** | **0 MB** | **132 MB** | 🏆 **Struct 最佳解**，停頓降 76%、0 LOH、記憶體減半 |
| ⚡ **A 級** | **2. 巢狀結構**<br>*(Struct 20k 筆)* | **ArrayPool (池化)** | `/api/members` | 3,272 | 107.4 ms (1.94%) | 12.1M | ⚠️ 51 次 (暖機高) | 69 MB | 276 MB | ⚡ **需隨機存取首選**，資料內嵌於 Buffer 完整池化 |
| ❌ **D 級** | **2. 巢狀結構**<br>*(Struct 20k 筆)* | **List (未池化)** | `/api/members-list` | 3,120 | **246.3 ms (4.25%)** | 45.9M | ❌ **202 次 (劇烈)** | 31 MB | 252 MB | ❌ **GC 停頓極長 (246ms)**，短命陣列引發頻繁 Full GC |
| 🛡️ **B 級** | **3. 參考型別**<br>*(Class 20k 筆)* | **Streaming (串流)** | `/api/members-class-stream` | 4,453 | **55.4 ms (1.02%)** | **5.2M** | ✅ **17 次 (常規)** | **0 MB** | **140 MB** | 🏆 **Class 最佳解**，GC 停頓降 70%，記憶體維持極低 |
| ⚠️ **C 級** | **3. 參考型別**<br>*(Class 20k 筆)* | **ArrayPool (池化)** | `/api/members-class-pooled` | 3,128 | 137.1 ms (2.53%) | 0.4M | ❌ **164 次 (劇烈)** | 6 MB | 258 MB | ⚠️ **池化效益低**，僅省下指標，物件依舊觸發長時間 GC |
| ❌ **D 級** | **3. 參考型別**<br>*(Class 20k 筆)* | **List (未池化)** | `/api/members-class-list` | 3,191 | 182.7 ms (3.24%) | 0.4M | ❌ **60 次 (劇烈)** | 4 MB | 229 MB | ⚠️ 4 萬個 Class 實體散落 Gen0，GC 停頓高達 182ms |
...
| 推薦等級 | 資料型別分類 | 實作架構 | API 端點 | 總耗時<br>(ms) | GC 總停頓時間<br>(Pause Time / 佔比) | Gen0 GC<br>次數 | Gen2 GC<br>次數 | LOH 峰值<br>(MB) | Working Set<br>實體記憶體 | 核心評語與行為特徵 |
|:---:|:---|:---|:---|:---:|:---:|:---:|:---:|:---:|:---:|:---|
| 🥇 **S 級** | **Struct / double** | **Streaming (串流)** | `/api/*-stream` | **3.3~3.6s** | **10.6~58.8 ms (極短)** | **1.3M~6.0M** | ✅ **16~19 次 (常規)** | **0 MB** | **114~132 MB (減半)** | 🏆 **效能與記憶體雙冠王**，全程 0 大物件、零暖機成本 |
| 🥈 **A 級** | **Struct / double** | **ArrayPool (池化)** | `/api/readings`, `/api/members` | 3.2~4.4s | 61.2~107.4 ms | 12.1M~75.5M | ⚠️ **19~51 次 (暖機後 0)** | 69~118 MB | 276~325 MB (常駐) | ⚡ **需隨機存取首選**，Gen0 暴降 99%，零後續 GC 停頓 |
| 🥉 **B 級** | **Class (參考型別)** | **Streaming (串流)** | `/api/members-class-stream` | 4.4s | **55.4 ms (極短)** | **5.2M** | ✅ **17 次 (常規)** | **0 MB** | **140 MB (減半)** | 🛡️ **既有 Class 模型無法改為 struct 時的最佳解** |
| ⚠️ **C 級** | **Class (參考型別)** | **ArrayPool (池化)** | `/api/members-class-pooled` | 3.1s | 137.1 ms (偏長) | 0.4M | ❌ **164 次 (劇烈)** | 6 MB | 258 MB (偏高) | ❌ **白做工**，只池化到指標，物件實體依舊在 Gen0 瘋狂產出垃圾 |
| 🚫 **D 級** | **Struct / Class / double** | **List (未池化)** | `/api/*-list` | 3.1~4.8s | **59.9~246.3 ms (極長)** | 0.4M~45.9M | ❌ **17~202 次 (極高)** | 31~62 MB | 196~252 MB | 💥 **效能毒藥**，擴容放大效應引發頻繁 Gen2 GC 與 OOM 風險 |
