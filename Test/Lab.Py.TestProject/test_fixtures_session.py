...
class TestCalculator:
    ...
    def test_subtract(self, setup_and_cleanup_session):
        target = Calculator()
        assert target.subtract(5, 3) == 2

    def test_multiply(self, setup_and_cleanup_session):
        target = Calculator()
        assert target.multiply(4, 3) == 12

    def test_divide(self, setup_and_cleanup_session):
        target = Calculator()
        assert target.divide(10, 2) == 5

    def test_divide_by_zero(self, setup_and_cleanup_session):
        target = Calculator()
