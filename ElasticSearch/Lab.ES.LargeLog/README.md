...
- **SQL (正向儲存 / B+ Tree)**：以「列 (Row)」為核心，搜尋 `LIKE '%關鍵字%'` 需逐筆全表掃描 (Full Table Scan)，資料量大時極慢。
- **ES (倒排索引 / Inverted Index)**：寫入時先將文字分詞（Tokenize），建立「詞彙指向文件 ID 清單」的字典。搜尋時直接查字典，毫秒級返回命中結果，無需掃描全表。
