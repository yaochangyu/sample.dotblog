...
更完整的模型比較可參考 [MTEB Leaderboard](https://huggingface.co/spaces/mteb/leaderboard)。它是目前最常被引用的 embedding benchmark 之一；實際選型時，建議優先看 **multilingual** 與 **retrieval** 任務成績，而不是只看總榜平均。
...
資料不是直接丟進 Embedding 就好，**怎麼切塊** 往往和模型選型一樣重要。

切太小，語意上下文不足；切太大，主題被稀釋，還可能超過模型的有效輸入長度。實務上通常會以**段落、標題、小節**為邊界，再視情況加上 overlap，避免重要資訊被切斷。

| 切法 | 優點 | 風險 |
|------|------|------|
| 固定字數切塊 | 簡單、好實作 | 容易切斷語意 |
| 依段落 / 標題切塊 | 語意較完整 | 需先解析文件結構 |
| 加 overlap | 降低邊界資訊遺失 | 儲存成本變高、重複內容增加 |

一個常見原則是：**讓每個 chunk 只表達一個主題，且查詢時能被單獨命中。**

---

### 5. Payload / Metadata 設計

向量資料庫存的不是只有向量，還有 payload。這些欄位會直接影響 filtering、權限控制、來源追蹤與結果可解釋性。
...
- `updated_at`：更新時間
- 業務欄位：例如 `category`、`author`、`region`

```json
{
  "doc_id": "handbook-001",
  "chunk_id": 12,
  "source": "/docs/employee-handbook.pdf",
  "language": "zh-TW",
  "section": "請假規範",
...
如果 payload schema 一開始沒設計好，後面要補 filter、回溯來源或做資料治理時，通常會很痛苦。

---

### 6. Hybrid Search（混合搜尋）
...
dense_model  = TextEmbedding(model_name="intfloat/multilingual-e5-small")
...
如果你的 embedding model 有要求 `query:` prefix、instruction 或 query / document 分流格式，**存入與查詢時都要一致**，否則檢索品質會明顯下降。
...
### 7. Quantization（向量量化壓縮）

大規模部署的成本關鍵。

1536 維 float32 向量每筆佔 6KB，百萬筆 = 6GB RAM。
這裡講的是**向量壓縮 / vector quantization**，不是 LLM 或 embedding model 本體的模型量化。常見做法會把向量壓成 `int8` 或更低位元格式，以空間換取速度與成本；代價是精準度通常會有些微下降。
...
### 問題一：近似搜尋有 Recall 上限
HNSW 是「近似」搜尋，不保證每次都找到真正最近的向量。
在資料量大、查詢困難或參數未調好的情境下，Recall 可能明顯低於 Full Scan，因此實務上通常需要搭配參數調校、Hybrid Search 或 Reranking 一起看。
...
### 問題四：Filtering 可能削弱 ANN 優勢
加上 metadata filter 後，搜尋效能不一定會維持原本的 ANN 優勢。特別是在 filter 選擇性很高、資料分布不平均時，可搜尋範圍變小，延遲與召回率都可能受到影響。

### 問題五：RAG 幻覺不是向量搜尋單獨能解決的
向量搜尋只負責「找出候選資料」，不保證最終生成一定正確；LLM 仍可能對檢索到的內容做錯誤推論或過度延伸。
**檢索品質好，不代表生成結果一定正確；檢索層與生成層需要分開評估。**
...
| 指標 | 問的問題 | 常見用途 |
|------|----------|----------|
| **Recall** | 相關的有沒有都找出來？ | 看系統會不會漏資料 |
| **Precision** | 找出來的有沒有都是相關的？ | 看結果雜訊多不多 |
| **Recall@k / Hit Rate@k** | 前 k 筆內有沒有找回相關答案？ | 搜尋、RAG Top-k 驗證 |
| **MRR** | 第一個正確答案出現得夠不夠前面？ | 問答、FAQ、已知單一答案任務 |
| **NDCG@k** | 排名前面的結果是不是更相關？ | 一般檢索排序品質評估 |

兩者通常是 trade-off：找越廣，準確率越低；找越準，可能漏掉重要資料。

實務上，評估 embedding model、chunking、Hybrid Search 或 reranking 時，最好固定同一批測試 query 與標註答案，再用這些指標做比較，不要只靠主觀感覺。
