...
public class ChangeTrackingUnitTest
{
    ...
    private readonly EmployeeAggregate _employeeAggregate = TestAssistants.EmployeeAggregate;
    ...
    {
        // var fromDb = Insert();
        // var employeeEntity = new EmployeeValueObject();
    ...
    {
        var fromDb = Insert();
        var target = this._employeeAggregate;
        target.SetEntity(fromDb.To())
            .SetProfile("小章", 28);
        target.AcceptChanges();
        var result = target.CommitChangeAsync().Result;
    }
