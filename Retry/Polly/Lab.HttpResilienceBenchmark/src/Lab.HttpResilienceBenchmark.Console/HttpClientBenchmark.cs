...
public class HttpClientBenchmark
{
    ...
    public void Setup()
    {
        ...
        // 設定標準 HTTP 客戶端（基準）
        services.AddHttpClient("standard", client => { client.BaseAddress = new Uri(_baseUrl); });

        // 設定 Resilience 客戶端 (Microsoft.Extensions.Http.Resilience)
        services.AddHttpClient("resilience", client => { client.BaseAddress = new Uri(_baseUrl); })
            .AddStandardResilienceHandler();

        // 設定 Polly V8 客戶端 (使用新的 ResiliencePipelineBuilder)
        services.AddHttpClient("pollyV8", client => { client.BaseAddress = new Uri(_baseUrl); })
