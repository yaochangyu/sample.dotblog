...
| 操作 | 行為 |
|------|------|
| 開啟 CSP | 動態注入 `<meta http-equiv="Content-Security-Policy">` tag，**無需重新載入頁面** |
| 關閉 CSP | 需要重新載入（瀏覽器安全模型限制：CSP 一旦套用，同一頁面生命週期內無法移除），但不會觸發 Session 自動恢復 |

預設狀態為 CSP 關閉，方便先觀察無防護時的攻擊效果，再啟用 CSP 驗證防護。
...
瀏覽器一旦套用 CSP（不論來自 Response Header 或 meta tag），就無法在同一個頁面生命週期內移除。因此模擬器的「關閉 CSP」操作仍需 reload 頁面，這是瀏覽器安全模型的設計，而非實作缺陷。
