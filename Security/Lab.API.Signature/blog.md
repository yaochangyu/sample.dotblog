...
> **注意**：本文描述的是進階版設計（9 欄位待簽字串、Redis Nonce 去重、金鑰管理服務與雙金鑰輪替等），與本資料夾 `Lab.API.Signature/` 實際的教學 Lab 程式碼（6 欄位 Canonical String、`IMemoryCache` Nonce、單一固定金鑰）**不是同一套實作**，屬於延伸參考文章，請勿依此文件對照除錯目前的程式碼。實際程式碼的規格請見 [`README.md`](./README.md)。
