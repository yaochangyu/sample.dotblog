...
## HybridCache L2 序列化器比較（1000 筆）

> 完整報告見 `hybridcache-serializer-report.md`
...
|---|---|---|---|
| data field 大小 | **225 KB** | 359 KB | MessagePack（少 37.4%）|
| MEMORY USAGE 累計 | **500 KB** | 625 KB | MessagePack |
| **used_memory 差值** | **576 KB** | **595 KB** | **MessagePack（少 3.2%）** |
| 純序列化速度（1000筆） | 32.71 ms | **7.99 ms** | MemoryPack（快 4.1x）|
| L2 寫入總耗時 | 255.73 ms | **200.18 ms** | MemoryPack |

### 關鍵觀察

- **data field 差 37%，但 `used_memory` 差值只差 3.2%**：Hash overhead（`absexp`、`sldexp`）與 jemalloc 對齊對兩種格式是固定成本，稀釋了序列化格式本身的差異。
- **序列化格式省了多少空間 ≠ Redis 真正省了多少空間**，應以 `used_memory` 差值為準。
- **速度**：MemoryPack 序列化快 4.1x，但 L2 寫入以 Redis I/O 為主，差距縮小為 1.3x。
