...
應用程式會在 `http://localhost:5178` 啟動。

#### 3. 開啟 API 文件

在瀏覽器中開啟 Swagger UI：http://localhost:5178/swagger
...
**測試指令（HTTP Request 格式）：**

```http
### 測試 max-age 指令
GET http://localhost:5178/api/clientcache/max-age
```
...
{
  "requestId": 1,
  "timestamp": "2026-01-08T02:30:15.267Z",
  "value": "此回應可快取 60 秒",
...
```http
### 測試 no-store 指令
GET http://localhost:5178/api/clientcache/no-store
```
...
{
  "requestId": 2,
  "timestamp": "2026-01-08T02:30:16.680Z",
...
```http
### 測試 ETag 強驗證（首次請求）
GET http://localhost:5178/api/clientcache/etag-strong
```
...
{
  "requestId": 3,
  "timestamp": "2026-01-08T02:30:18.812Z",
  "etag": "\"v1.0\"",
  "contentVersion": "v1.0"
}
```
...
```http
### 測試 no-cache 指令
GET http://localhost:5178/api/clientcache/no-cache
```
...
{
  "requestId": 5,
  "timestamp": "2026-01-08T02:30:20.636Z",
  "etag": "\"5\"",
...
```http
### 測試 Vary 標頭
GET http://localhost:5178/api/clientcache/vary
```
...
{
  "requestId": 6,
  "timestamp": "2026-01-08T02:30:22.519Z",
  "acceptEncoding": "none",
...
```http
### 測試 immutable 指令
GET http://localhost:5178/api/clientcache/immutable
```
...
{
  "requestId": 7,
  "timestamp": "2026-01-08T02:30:24.015Z",
...
GET http://localhost:5178/api/clientcache/stale-while-revalidate
```
...
{
  "requestId": 8,
  "timestamp": "2026-01-08T02:30:25.477Z",
...
```http
### 查看伺服器統計資訊
GET http://localhost:5178/api/clientcache/stats
```
...
{
  "totalRequests": 8,
  "serverStartTime": "2026-01-08T02:30:10.775Z",
  "uptime": "00:00:30.199",
...
```http
### 一般請求（會使用快取）
GET http://localhost:5178/api/clientcache/client-controlled
```

**第一次請求（無 Cache-Control）回應：**
```http
HTTP/1.1 200 OK
Cache-Control: public, max-age=60
...
**第二次請求（帶 Cache-Control: no-cache）回應：**
```http
HTTP/1.1 200 OK
Cache-Control: public, max-age=60
...
這個實驗展示了如何結合 HTTP Client Cache 與伺服器端快取（HybridCache）來實現完整的多層快取架構。
...
```http
### 首次請求文章（會從資料庫讀取並快取）
GET http://localhost:5178/api/clientcache/article/1
```
...
**觀察結果：**
- 這是第一次請求，伺服器從資料來源讀取（可能經過 HybridCache 快取）
- 回應中包含 `ETag` 和 `Cache-Control`
- 瀏覽器會快取此回應 60 秒
...
#### 實驗 10-3：60 秒後再次請求（使用 ETag 驗證）

**測試指令：**
```http
### 60 秒後再次請求（使用 If-None-Match 進行條件請求）
GET http://localhost:5178/api/clientcache/article/1
...
**觀察結果：**
- 瀏覽器發送條件請求（帶 `If-None-Match` 標頭）
- 伺服器比對 ETag，發現內容未改變
- 回傳 **304 Not Modified**，無 body
- 瀏覽器使用本地快取的內容

**總耗時：** ~5-10ms（網路往返 + 伺服器處理）
...
```http
### 更新文章內容
PUT http://localhost:5178/api/clientcache/article/1
Content-Type: application/json
...

{
  ...
  "article": {
    ...
    "title": "深入探討 HTTP Client-Side Cache（已更新）",
    "content": "更新後的內容...",
    "updatedAt": "2026-01-08T02:35:00Z"
  },
  "newETag": "\"638734985000000000\""
}
```
...
- 文章內容已更新
- `UpdatedAt` 時間戳已改變
- ETag 值已改變（因為基於 `UpdatedAt.Ticks`）
- 伺服器端快取已清除（如果使用 HybridCache）

#### 實驗 10-5：使用舊 ETag 請求已更新的文章
...
**觀察結果：**
- 客戶端的 ETag (`"638734982345678901"`) 與伺服器的新 ETag (`"638734985000000000"`) **不匹配**
- 伺服器回傳 **200 OK** + 完整的新資料
- 客戶端更新本地快取並使用新的 ETag
...
時間軸：
0s ~ 60s: 瀏覽器直接使用快取，不發送請求
60s+:     瀏覽器發送條件請求（If-None-Match）
          ├─ 內容未變更 → 304 Not Modified
          └─ 內容已變更 → 200 OK + 新資料 + 新 ETag
```
...
GET http://localhost:5178/api/clientcache/http-response-cache
```
...
GET http://localhost:5178/api/clientcache/http-cache-control
```
...
```http
### 測試動態快取（一般使用者）
GET http://localhost:5178/api/clientcache/dynamic-cache?isVip=false

### 測試動態快取（VIP 使用者）
GET http://localhost:5178/api/clientcache/dynamic-cache?isVip=true
```
...
```http
### 首次請求資料
GET http://localhost:5178/api/clientcache/data/1
...
```http
### 測試固定快取策略
GET http://localhost:5178/api/products
```
...
```http
### 測試新發布的新聞（快取 1 分鐘）
GET http://localhost:5178/api/news/1

### 測試一週內的新聞（快取 30 分鐘）
GET http://localhost:5178/api/news/2

### 測試舊新聞（快取 24 小時）
GET http://localhost:5178/api/news/3
```
...
```http
### 測試多語系端點（正常情況）
GET http://localhost:5178/api/localized
...
```http
### 測試使用者個人資料（快取 1 小時）
GET http://localhost:5178/api/profile

### 測試產品列表（快取 5 分鐘）
GET http://localhost:5178/api/products

### 測試即時股票價格（不快取）
GET http://localhost:5178/api/stock-price
```
...
```http
### 首次請求文章
GET http://localhost:5178/api/article/1
...
```http
### 請求繁體中文版本
GET http://localhost:5178/api/data
Accept-Language: zh-TW
...
本專案展示了如何結合 HTTP Client-Side Cache 與伺服器端 HybridCache，實現完整的多層快取架構。
...
更多關於 HybridCache 的詳細資訊，請參考：[CACHE-ARCHITECTURE.md](./CACHE-ARCHITECTURE.md)
