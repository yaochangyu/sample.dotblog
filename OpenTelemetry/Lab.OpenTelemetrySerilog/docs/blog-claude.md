...
├─ Span: frontend.fetch /api/weather
...
Nuxt Server API 需要將前端的 `traceparent` header 透傳給 Backend-A：
...
| 驗證項目 | 操作方式 |
|---------|---------|
| 分散式追蹤 | 開啟 Jaeger UI（http://localhost:16686），查看 frontend → backend-a → backend-b 的完整 Trace |
